...
const vendorPath = upath.normalizeSafe(argv.vendorPath || './vendor/sylius/sylius/src/Sylius/Bundle');
...
const customPath = upath.normalizeSafe('./assets/shop/custom');
    ...
    plugins: [
      ...
      babel({
        ...
        plugins: [
          ...
          ['module-resolver', {
            alias: {
              'sylius/ui': upath.joinSafe(vendorUiPath, 'Resources/private/js'),
            },
          }],
...
export const buildCustomJs = async function buildCustomJs() {
  const bundle = await rollup({
    input: upath.joinSafe(customPath, '/js/custom.js'),
    plugins: [
      {
        name: 'shim-app',

        transform(code, id) {
          if (upath.relative('', id) === upath.joinSafe(customPath, '/js/**')) {
            return {
              code: dedent`
                import './shim/shim-polyfill';
                import './shim/shim-jquery';
  ...
  await bundle.write({
    file: upath.joinSafe(shopRootPath, 'js/custom.js'),
    format: 'iife',
    sourcemap: options.sourcemaps,
  });
};
buildCustomJs.description = 'Build shop js assets.';
...
export const buildFullJs = gulp.parallel(buildShopJs, buildCustomJs);
buildFullJs.description = 'Watch native and custom js.';

export const watchShop = function watchShop() {
  livereload.listen();

  gulp.watch(paths.shop.js, buildFullJs);
